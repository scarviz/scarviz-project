...
public class BattleMainActivity extends BaseActivity {
    ...
    // 攻撃最大値
    private static final int ATTACK_MAX = 50000;
...
/*		// TODO : 攻撃ゲージがMaxになるまでまわす
		// TODO : タイマー使った方がいいかも？一定時間回すようにするとか
		while(true){
			// 音量を取得し、加算する
			int vol = mVolumeDetection.GetVolume();
			mMyVolume += vol;
			Log.i("one-volume",String.valueOf(vol));
			Log.i("volume",String.valueOf(mMyVolume));
			// パワーゲージを更新する
			SetPoewr();
			// Max値以上になった場合
			if(mMyVolume >= ATTACK_MAX){
				break;
			}
	 ...
	 * 
	 */
	private void SetPoewr(){
		String poewr = "□□□□□□□□□□";
		int poewrValue = mMyVolume/ATTACK_MAX * 10;
		
		if(poewrValue <= 1){
			poewr = "■□□□□□□□□□";
		}
		else if(poewrValue <= 2){
			poewr = "■■□□□□□□□□";
		}
		else if(poewrValue <= 3){
			poewr = "■■■□□□□□□□";
		}
		else if(poewrValue <= 4){
			poewr = "■■■■□□□□□□";
		}
		else if(poewrValue <= 5){
			poewr = "■■■■■□□□□□";
		}
		else if(poewrValue <= 6){
			poewr = "■■■■■■□□□□";
		}
		else if(poewrValue <= 7){
			poewr = "■■■■■■■□□□";
		}
		else if(poewrValue <= 8){
			poewr = "■■■■■■■■□□";
		}
		else if(poewrValue <= 9){
			poewr = "■■■■■■■■■□";
		}
		else {
			poewr = "■■■■■■■■■■";
